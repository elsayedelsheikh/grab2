...
# TODO: Publish BBOX3D
# def publish_camera_bbox3d(camera: Camera, freq, topic_name, frame_id, node_namespace='')
# if bbox3d_pub:
...
def create_camera_graph(
    ...
    frequency: int = 30,
    camera_frame_id: str = 'eef_camera',
    node_namespace: str = 'eef_camera',
    ...
    publish_camera_info(
        camera_obj, frequency, topic_info, camera_frame_id, node_namespace
    )
    publish_camera_rgb(
        camera_obj, frequency, topic_rgb, camera_frame_id, node_namespace
    )
    ...
    if topic_depth is not None:
        publish_camera_depth(
            camera_obj, frequency, topic_depth, camera_frame_id, node_namespace
        )
...
def publish_camera_tf(
    camera_stage_path: str,
    camera_frame_id,
    graph_path: str = '/World/Graphs/Transforms',
):
        ...
        og.Controller.edit(
            graph_path,
            {
                ...
                og.Controller.Keys.SET_VALUES: [
                    ...
                    (
                        'PublishTF_' + camera_frame_id + '.inputs:targetPrims',
                        camera_stage_path,
                    ),
                ],
                og.Controller.Keys.CONNECT: [
...
def create_jointstates_graph(
    robot_stage_path: str,
    articulation_root_path: Optional[str] = None,
    graph_stage_path: str = '/World/Graphs/JointStateGraph',
):
...
def add_franka(
    ...
    # Camera
    camera_stage_path = (
        f'{robot_stage_path}/panda_hand/geometry/realsense/realsense_camera'
    )
    # Fix camera settings since the defaults in the realsense model are inaccurate
    realsense_prim = UsdGeom.Camera(
        stage.get_current_stage().GetPrimAtPath(camera_stage_path)
    )
...
def add_ur10e_with_suction_cup(
    robot_stage_path: str,
    position: list[float] = (0.0, 0.0, 0.0),
    use_jointstates_action_graph: bool = True,
    use_camera_action_graph: bool = True,
    ...
    ROBOT_USD_PATH = os.path.join(
        current_dir, 'assets', 'robots', 'ur10e_short_suction.usd'
    )
    ...
    # prims
    suction_gripper_stage_path = f'{robot_stage_path}/ee_link'
    camera_stage_path = f'{suction_gripper_stage_path}/Camera'

    # Action graphs
    if use_jointstates_action_graph:
        create_jointstates_graph(robot_stage_path, robot_stage_path + '/root_joint')
    if use_camera_action_graph:
        create_camera_graph(camera_stage_path)
